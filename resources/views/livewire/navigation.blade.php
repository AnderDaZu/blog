                ...
                <div class="hidden sm:block sm:ml-6">
                    <div class="flex space-x-4">
                        ...
                        @foreach ($categories as $category)
                            <a href="{{ route('posts.category', $category->id) }}" class="text-gray-300 hover:bg-gray-700 hover:text-white px-3 py-2 rounded-md text-sm font-medium">{{ $category->name }}</a>
                        @endforeach
                    </div>
                </div>
            </div>
                    ...
                    <div class="ml-3 relative">
                        ...
                        <div x-show="open" x-on:click.away="open=false" class="origin-top-right absolute right-0 mt-2 w-48 rounded-md shadow-lg py-1 bg-white ring-1 ring-black ring-opacity-5 focus:outline-none"
                            ...
                            <a href="{{ route('profile.show') }}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1"
                                id="user-menu-item-0">Tu pérfil</a>

                            <form method="POST" action="{{ route('logout') }}">
                                @csrf
                                <a href="{{ __('Log Out') }}" class="block px-4 py-2 text-sm text-gray-700" role="menuitem" tabindex="-1"
